defmodule Bookkeeping.Boundary.AccountingJournal.Server do
  ...
  #         posting_date_details => list(JournalEntry.t())
  #       }
  ...
  #     iex> Bookkeeping.Boundary.AccountingJournal.Server.find_journal_entries_by_posting_date(~U[2021-10-10 10:10:10.000000Z])
  #     {:ok, [%JournalEntry{...}, %JournalEntry{...}, ...]}

  #     iex> Bookkeeping.Boundary.AccountingJournal.Server.find_journal_entries_by_posting_date(%{year: 2021, month: 10})
  #     {:ok, [%JournalEntry{...}, %JournalEntry{...}, ...]}
  ...
  #         DateTime.t() | posting_date_details()
  #       ) :: {:ok, list(JournalEntry.t())} | {:error, :invalid_date}
  # def find_journal_entries_by_posting_date(server \\ __MODULE__, datetime) do
  #   GenServer.call(server, {:find_journal_entries_by_posting_date, datetime})
  # end
  ...
  #     iex> Bookkeeping.Boundary.AccountingJournal.Server.find_journal_entries_by_posting_date_range(~U[2021-10-10 10:10:10.000000Z], ~U[2021-10-10 10:10:10.000000Z])
  #     {:ok, [%JournalEntry{...}]}

  #     iex> Bookkeeping.Boundary.AccountingJournal.Server.find_journal_entries_by_posting_date_range(%{year: 2021, month: 10, day: 10}, %{year: 2021, month: 10, day: 10})
  #     {:ok, [%JournalEntry{...}]}
  ...
  #         DateTime.t() | posting_date_details()
  #       ) :: {:ok, list(JournalEntry.t())} | {:error, :invalid_date}
  # def find_journal_entries_by_posting_date_range(
  #       server \\ __MODULE__,
  #       from_datetime,
  #       to_datetime
  #     ) do
  #   GenServer.call(
  #     server,
  #     {:find_journal_entries_by_posting_date_range, from_datetime, to_datetime}
  #   )
  # end
  ...
  #     iex> Bookkeeping.Boundary.AccountingJournal.Server.update_journal_entry(%JournalEntry{...}, %{description: "updated description",posted: true})
  ...
  #          ) do
  #     date_details = Map.take(journal_entry.posting_date, [:year, :month, :day])
  ...
  # @impl true
  # def handle_call(
  #       {:find_journal_entries_by_posting_date, datetime},
  #       from,
  #       journal_entries
  #     ) do
  #   Task.async(fn ->
  ...
  #   date_details =
  #     Map.take(updated_journal_entry.posting_date, [:year, :month, :day])

  #   if journal_entries[date_details] == nil do
  #     with {:ok, old_journal_entry} <- find_by_id(journal_entries, updated_journal_entry.id),
  #          {:ok, old_date_details} <-
  #            get_date_details(old_journal_entry.posting_date) do
  #       updated_je_list =
  ...
  #         {:error, message} ->
  #           errors =
  #             acc.error ++ [%{document_number: params.journal_entry_number, error: message}]
  ...
  #     reference_number = Map.get(csv_item, "Transaction Reference Number", "")
  #     csv_posted = Map.get(csv_item, "Posted", "no")
  #     description = Map.get(csv_item, "Journal Entry Description", "")
  #     particulars = Map.get(csv_item, "Journal Entry Details", "{}")
  ...
  #          {:ok, particulars} <- Jason.decode(particulars),
  ...
  #     else
  #       {:error, error} ->
  #         errors = acc.error ++ [%{document_number: journal_entry_number, error: error}]
  #         Map.put(acc, :error, errors)

  #       _error ->
  #         errors =
  #           acc.error ++ [%{document_number: journal_entry_number, error: :invalid_csv_item}]
  ...
  #     is_binary(reference_number) and is_binary(description) and
  #     is_binary(particulars) and is_binary(audit_details) and is_binary(csv_posted) and
  ...
  #     existing_description != "" and description == "" ->
  #       existing_description
  ...
  #         %{document_number: document_number} when document_number == journal_entry_number ->
